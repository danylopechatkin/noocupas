...
const VisualPlaceholder: React.FC<{ dark?: boolean; label?: string }> = ({ dark = false, label = "Product image placeholder" }) => (
  <div className={`relative isolate w-full ${dark ? "bg-black" : "bg-white"}`}>
    <div className="aspect-[2/1] md:aspect-[21/9] lg:aspect-[16/7] w-full overflow-hidden rounded-3xl">
      <div className={`h-full w-full ${dark ? "bg-[#0A0A0A]" : "bg-[#F5F5F7]"} relative`}>
        ...
        <div className={`absolute inset-6 rounded-3xl border ${dark ? "border-white/5" : "border-black/5"}`} />
...
const HeroDark: React.FC = () => (
  <section id="products" className="bg-gradient-to-b from-[#0a0a0a] to-[#1a1a1a] text-white">
  ...
  <section id="solutions" className="bg-[#F5F5F7] text-black">
    <Container className="max-w-[980px] px-4 md:px-6 text-center">
      <div className="pt-20 pb-8 md:pt-24 md:pb-12">
        <h2 className="text-[40px] md:text-[56px] leading-[0.95] font-bold tracking-[-0.02em]">NOOCUPAS Air</h2>
...
const Tile: React.FC<{
  ...
  return (
    <div
      className={`relative min-h-[700px] rounded-3xl overflow-hidden p-8 md:p-12 lg:p-16 ${
        isDark ? "bg-[#0A0A0A] text-white" : isSoft ? "bg-[#F5F5F7] text-black" : "bg-white text-black"
      }`}
    >
      <div className="max-w-[560px]">
        <h3 className="text-[32px] md:text-[44px] font-bold tracking-[-0.02em]">{title}</h3>
        {subtitle && (
          <p className={`mt-3 text-base md:text-lg ${isDark ? "text-white/80" : "text-neutral-700"}`}>{subtitle}</p>
        )}
      </div>
        ...
        {[
          ...
          <div key={col.h}>
            ...
            <ul className="space-y-2">
              {col.i.map((x) => (
                <li key={x} className="cursor-pointer text-sm text-neutral-700 hover:text-black">
                  {x}
                </li>
              ))}
            </ul>
